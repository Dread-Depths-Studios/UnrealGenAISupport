...
def handle_take_screenshot(command):
    """
    Issues a HighResShot console command and returns the expected file path.

    HighResShot is async — the render thread captures on the next tick. The
    handler must NOT block the game thread polling for the file (doing so
    prevents the very tick that produces the screenshot). Callers poll the
    returned path themselves.
    """
    temp_dir = tempfile.gettempdir()
    unique_filename = f"unreal_mcp_screenshot_{int(time.time() * 1000)}.png"
    # Forward slashes are more reliable for Unreal console commands.
    ...
    try:
        ...
        return {"success": False, "error": f"Screenshot command failed: {str(e)}"}
